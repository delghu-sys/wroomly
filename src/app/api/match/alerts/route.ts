...
import { normalizeCriteria, humanizeCriteria } from '@/lib/match/criteria'
 ...
 *
 * Creates (or refreshes) a Wroomly Match alert. Single opt-in: the renter typed
 * their own email, so it activates immediately and we send a welcome email with
 * manage + unsubscribe links. The opt-in also joins the renter launch list.
 *
 * Public + anonymous. The returned manage token lets the renter edit/unsubscribe
 * without an account.
 */
const bodySchema = z.object({
  email: z.string().email().max(200),
  // Validated loosely here; normalizeCriteria is the real guard.
  criteria: z.unknown(),
  transcript: z
    .array(
      z.object({
...
export async function POST(request: Request) {
  ...
  const tags = humanizeCriteria(criteria)
  void (async () => {
    const { subject, html } = matchWelcomeEmail({ tags, token: alert.manage_token })
    await sendEmail({
      to: alert.email,
      subject,
      html,
