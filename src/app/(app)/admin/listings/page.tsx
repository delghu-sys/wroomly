  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-3xl tracking-tight text-ink">Listings</h1>
          ...
          <Link
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              status === tab.value
                ...
                : 'bg-surface text-ink-soft hover:bg-navy-soft'
            }`}
          >
            {tab.label}
          </Link>
      ...
      {listings.length === 0 ? (
        ...
        <div className="space-y-4">
          {listings.map(listing => {
            ...
            return (
              <div key={listing.id} className="bg-surface border rounded-2xl p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-semibold text-ink">{listing.title}</h3>
                      <Link href={`/listings/${listing.id}`} target="_blank">
                        <ExternalLink className="w-3.5 h-3.5 text-ink-muted hover:text-[#2F6BFF]" />
                      </Link>
                    </div>
                ...
                <div className="flex items-center gap-2 mb-4">
                  <Avatar className="h-7 w-7">
                    ...
                    <AvatarFallback className="text-xs bg-navy-soft text-navy">{initials}</AvatarFallback>
                  </Avatar>
                  <div className="text-sm">
                    <span className="font-medium text-ink-soft">{supplier?.full_name}</span>
                    <span className="text-ink-muted ml-2">{(supplier as { email?: string } | null)?.email}</span>
                  </div>
                </div>

                {listing.description && (
                  <p className="text-sm text-ink-soft line-clamp-2 mb-4">{listing.description}</p>
                )}
