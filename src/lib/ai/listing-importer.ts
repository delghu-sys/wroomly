...
- Classify campusArea from text when possible, else UNKNOWN. The "address" field must be a COMPLETE street address (house number + street name) or null — nothing in between. If you only know part of it (e.g. a house number but not the street, or just a neighborhood/building/area name), leave "address" null and put what you do know in "neighborhood", "campusArea", or "buildingName" instead. NEVER write a partial address, a guess, or a placeholder like "[Street unknown]" or "123 [unknown], Ann Arbor" into "address" — the app asks the user to search for and confirm their exact address separately, so a null here is correct and expected, not a failure.
