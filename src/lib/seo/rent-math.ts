/**
 * Pure rent/availability math. NO server imports on purpose: this is the code
 * that decides the numbers published on /llms.txt, the rent guide and all 11
 * neighborhood pages, so it has to be unit-testable. `rent-stats.ts` is the
 * thin server layer that feeds it live rows and re-exports everything here.
 */

/** The shape the maths needs. `RentSampleRow` satisfies this structurally. */
...
function bucketMedian(rows: PricedRow[]): number | null {
 ...
 */
export function computeAvailability(rows: PricedRow[]): AvailabilityStats {
  ...
  // getUTCMonth, NOT getMonth. `available_from` is a calendar date ("2026-08-01")
  // which Date parses as UTC midnight, while getMonth() reads LOCAL time — so
  // anywhere west of UTC (including America/Detroit, Ann Arbor's own zone) an
  // August 1st listing was counted as July. That silently shifted every
  // first-of-month listing back a month in a published statistic.
  for (const v of valid)
    counts.set(v.from.getUTCMonth(), (counts.get(v.from.getUTCMonth()) ?? 0) + 1)
...
/**
 * Everything except availability, computed from already-priced rows.
 * Split from availability so each can be tested in isolation.
 */
export function summarizeRows(rows: PricedRow[]): Omit<RentStats, 'availability'> {
  const prices = rows.map(r => r.price_per_month!).filter(Boolean)
  ...
  for (const [label, match] of BEDROOM_BUCKETS) {
    ...
    // Exact divisor only: "4+" mixes 4, 5, 6 and 7-bed units, so a
    // per-bedroom figure for it would be meaningless.
