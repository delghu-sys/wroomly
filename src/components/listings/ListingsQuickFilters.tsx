    ...
    <div className="flex items-center gap-3 flex-wrap">
      ...
      {activeFilters.map(({ key, value, label }) => (
        <button
          ...
          className="inline-flex items-center gap-1.5 h-7 pl-3 pr-2 rounded-full bg-maize-bright/15 text-[oklch(0.32_0.10_85)] text-xs font-medium hover:bg-maize-bright/25 transition-colors duration-300 group active:scale-[0.97] focus:outline-none focus-visible:ring-4 focus-visible:ring-maize-bright/30"
        >
          {label}
      ...
      {activeFilters.length > 1 && (
        <button
          onClick={clearAll}
          className="text-xs text-ink-muted hover:text-gold-deep font-medium transition-colors focus:outline-none focus-visible:underline underline-offset-4"
        >
          Clear all
        </button>
      )}
