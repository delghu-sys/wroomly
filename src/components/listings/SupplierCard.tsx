...
export function SupplierCard({ user, ratingAvg, reviewCount }: SupplierCardProps) {
  ...
  return (
    ...
    >
      <Link
        href={`/users/${user.id}`}
        className="group relative block rounded-3xl overflow-hidden border border-line bg-white/70 backdrop-blur-xl p-6 transition-shadow duration-500 hover:shadow-[0_18px_50px_oklch(0_0_0/0.08)]"
      >
            ...
            <div className="flex items-center gap-3 text-sm mt-1.5">
              ...
              {ratingAvg !== null && (
                <span className="inline-flex items-center gap-1 text-ink">
                  ...
                  <span className="font-medium tabular-nums">{ratingAvg.toFixed(1)}</span>
